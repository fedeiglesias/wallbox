  ...
  >
    ...
    >
      <transition name="fade">
        <div class="cover-overlay" v-if="actionsOpen"></div>
      </transition>
    </v-img>
      ...
      >
        ...
        <v-tooltip left color="rgba(0,0,0,0.9)">
          <template v-slot:activator="{ on }">
            <v-btn
              v-on="on"
              @click="($event) => showDeleteDialog(book.id)"
              fab
              dark
              small
              color="red"
    ...
    <v-card-subtitle class="subtitle">
      ...
      <span class="pages">
        <v-icon small color="rgba(0,0,0,0.4)"
    ...
    <div class="authors">
      <v-chip pill v-for="author in authors" :key="author.id">
        <v-avatar left>
          <v-img contain class="avatar" :src="author.avatar"> </v-img>
        </v-avatar>
